...
implementation can run under MPI, which can span nodes; nothing here says how
...
process: 543 MB, 2,965 MB and 5,686 MB at 512, 5,000 and 10,000 cells on one
core; 3,009 MB, 10,033 MB and 18,737 MB with 10 ranks, 5.5, 3.4 and 3.3 times
as much for 2.2, 1.5 and 1.7 times the speed. Under MPI there is one Python
process per rank, and the summed memory grows with the rank count. Running
under MPI also changes its result: RF 141 against 146 at 512 cells, 2,016
against 1,937 at 5,000 and 5,151 against 5,149 at 10,000.

Two cautions. The Rust quality differences between Sanity paths and search
modes are inside the run-to-run spread in `docs/PERFORMANCE.md` ("How much one real-data run
says"): the 0.45 against 0.57 at 5,000 cells is
